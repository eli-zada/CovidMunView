angular.module('myApp', ['ngSanitize', 'ui.select', 'n3-line-chart'])
    .controller('HomeCtrl', function($scope, $q, covidgraph, RestService, storageService) {
        ...
        $scope.loadGraphData = function() {
            ...
            covidgraph.getdatadaysback($scope.model.citySelected.code, $scope.graphDaysBack)
                .then(function(data) {
                    ...
                    $scope.casesTrend = $scope.calcCasesTrend();
        ...
        $scope.proccessData = function(covidArr, $scope) {
            ...
            $scope.covid['all'].active_cases = $scope.covid['all'].cases - $scope.covid['all'].recoveries;
            if ($scope.covid['all'].deaths != "<15") {
                $scope.covid['all'].active_cases -= $scope.covid['all'].deaths;
            }
            ...
            covidArr["agas"].forEach(covidObj => {
                ...
                $scope.covid[_neibor].active_cases = $scope.covid[_neibor].cases - $scope.covid[_neibor].recoveries;
                if ($scope.covid[_neibor].deaths != "<15") {
                    $scope.covid[_neibor].active_cases -= $scope.covid[_neibor].deaths;
                }
        ...
        $scope.calcCasesTrend = function() {

            let _arr = {};
            Object.keys($scope.graphData).forEach(function(key, index) {
                this[key].dataset.sort((a, b) => (a.x > b.x) ? 1 : ((b.x > a.x) ? -1 : 0));
                arrLen = this[key].dataset["length"];
                if (arrLen > 1) {
                    let _lastD = this[key].dataset[arrLen - 1]["case01"] - this[key].dataset[arrLen - 1]["recover01"];
                    let _beforeLastD = this[key].dataset[arrLen - 2]["case01"] - this[key].dataset[arrLen - 2]["recover01"];
                    _arr[key] = _lastD - _beforeLastD;
                } else {
                    _arr[key] = 0;
                }
                console.log(this[key].dataset, _arr[key]);
            }, $scope.graphData);
